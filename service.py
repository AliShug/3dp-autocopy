...
def show_message(msg):
    print(msg)
    ctypes.windll.user32.MessageBoxW(0, msg, '3DP Auto Loader', 1)

######################################################## Removable drive listing and ejection code from github.com/Ultimaker/Cura
# Minor modifications to make minimal working copies of these functions
...
with open('3dp_settings.json', 'r') as f:
    settings = json.load(f)
    settings['src'] = os.path.expandvars(settings['src'])
    settings['dst'] = os.path.expandvars(settings['dst'])

if not os.path.isdir(settings['src']):
    print('Source directory does not exist! Quitting')
    exit()
if os.path.exists(settings['dst']):
    print('Destination drive already exists! Quitting')
    exit()
...
while True:
    ...
    if drive_inserted:
        # Wait for removal
        if not settings['dst'] in drives:
            drive_inserted = False
            if len(files) != 0 and settings['eject']:
                show_message('Drive ejected, moved files: {}'.format(files))
    else:
        if settings['dst'] in drives:
            print('{} inserted'.format(drives[settings['dst']]))
            files = glob.glob(settings['src'] + '\\*')
            if len(files) == 0:
                show_message('Nothing to transfer')
            else:
                for file in files:
                    shutil.copy(file, settings['dst'])
                    os.unlink(file)
                if settings['eject']:
                    eject_device(settings['dst'])
                else:
                    show_message('Moved files: {}'.format(files))
            drive_inserted = True
